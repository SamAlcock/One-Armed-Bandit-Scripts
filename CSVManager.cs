...
public class CSVManager : MonoBehaviour
{
    ...
    public void Main()
    {
        ...
        for (int i = 1; i < Data.Count; i+=8) // Increment i by 8 because every 8 entries (excluding i = 0) is data for a new trial
        {
            // Assign this data to a new instance of ParticipantEntry
            participant.Add(new ParticipantEntry { Id = (int)Data[0], Trial = (int)Data[i], Score = (int)Data[i + 1], Explored = (int)Data[i + 2], Exploited = (int)Data[i + 3], ResponseTime = Data[i + 4], TooSlow = (int)Data[i + 5], ZKey = (int)Data[i + 6], MKey = (int)Data[i + 7]});
        }
        ...
        {
            sw.WriteLine("Participant Number,Trial Number,Score,Explored,Exploited,Response Time (seconds),Too Slow,Z Key,M Key");
        }
    ...
    void InputData(List<ParticipantEntry> participant, StreamWriter writer) // Inputs every instances attribute in the correct format
    {
        ...
        {
            writer.WriteLine(participant[i].Id + "," + participant[i].Trial + "," + participant[i].Score + "," + participant[i].Explored + "," + participant[i].Exploited + "," + participant[i].ResponseTime + "," + participant[i].TooSlow + "," + participant[i].ZKey + "," + participant[i].MKey);
        }
    }
